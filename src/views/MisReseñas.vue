      ...
      <v-row v-for="(review, index) in reseñas.data" :key="index" class="py-4">
        <v-col cols="12" md="4">
          <v-card flat height="100%">
            ...
            <v-img :aspect-ratio="16 / 9" height="100%" :src="review.linkImagen"></v-img>
          </v-card>
        </v-col>
...
export default {
  data() {
    return {
      ...
      categorias: ['Película', 'Videojuego', 'Producto', 'Electrodomésticos'], // Opciones para el selector de categorías
  ...
  methods: {
    mounted() {
      axios.get('http://localhost:4001/api/resenas/mis_resenas')
        .then(response => {
          this.reseñas = response.data.data;
        })
        .catch(error => {
    ...
    submitReview() {
      const nuevaResena = {
        ...
        productoID: this.productoID // ID del producto reseñado
      };

    axios.post('http://localhost:4001/api/resenas/crear_resena', nuevaResena)
    .then(response => {
      console.log('Reseña creada:', response.data);
      
      // Agrega la nueva reseña al arreglo de reseñas en el frontend
      this.reseñas.push(response.data.data);
      
      // Limpia los campos del formulario después de crear la reseña
      this.categoria = null;
      this.genero = '';
      this.precio = null;
      this.nombreElemento = '';
